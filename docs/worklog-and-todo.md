...
포트폴리오 첫 페이지 하단에 공개 저장소 URL
`https://github.com/jsoyun/point-payment-lab`을 추가했다. PDF에서도 바로 저장소를
열 수 있도록 클릭 가능한 링크 영역을 함께 설정했다.

표지의 프로젝트 역할 문구 앞에 작성자 이름 `이소윤`을 추가해 지원자가 누구인지
첫 페이지에서 바로 확인할 수 있도록 수정했다.
